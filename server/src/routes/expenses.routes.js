...
function validateExpenseInput(req, body) {
  const { title, currency, amount, paidBy, date, categoryId, categoryName, participantIds } = body || {};
...
router.post("/", (req, res) => {
  ...
  const { trimmedTitle, currency, amountCents, paidBy, date, category, participants } = result;
  ...
  const createExpense = db.transaction(() => {
    const info = db
      ...
      .run(req.project.id, category.id, trimmedTitle, currency, amountCents, paidBy, date, req.user.id);
...
router.put("/:expenseId", (req, res) => {
  ...
  const updateExpense = db.transaction(() => {
    db.prepare(
      `UPDATE expenses SET category_id = ?, title = ?, currency = ?, amount_cents = ?, paid_by = ?, expense_date = ?
       WHERE id = ?`
    ).run(category.id, trimmedTitle, currency, amountCents, paidBy, date, expense.id);
